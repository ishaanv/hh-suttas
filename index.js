...
function displaySuttas(suttas, isSearch = false) {
  ...
  let currentGroup = -1;
  suttaArea.innerHTML += `<ul style="margin-top: 20px;">${Object.entries(suttas)
    .map(([sutta_id, sutta_details]) => {
        ...
        html +=
          ...
          `${
            comment_text[segment]
              ? `<span class="comment">*<span class="comment-text" style="display: none;">${converter
                  .makeHtml(comment_text[segment])
                  .replace(/^<p>(.*)<\/p>$/, "$1")}</span></span>`
              : ""
          }` +
          `</span></span>${closeHtml}\n\n`;
      });
...
document.getElementById("cacheButton").addEventListener("click", () => {
  ...
  if ("serviceWorker" in navigator) {
    ...
    try {
      ...
      showNotification(
        "An error occurred while attempting to download. Please refresh the page, wait a few seconds, and retry"
      );
    }
  }
});
